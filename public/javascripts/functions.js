...
function searchBTN(){
    //if($("#article_name").val()||$("#article_name").val()||$("#article_height_length").val()||$("#article_height").val())
    //{
        var form_json = {data: [
                {category: $("#category").val()},
                {name: $("#article_name").val()},
                {width: $("#article_width").val()},
                {height_length: $("#article_height_length").val()},
                {height: $("#article_height").val()}
            ]
        };

        $.get('/search',form_json);
        window.location.href="/search?page=1";
    //}else{
    //    swal('Warning!','이름, 가로, 세로, 높이 중 1개 이상의 값이 필요합니다.');
    //}
}
function addBTN(){
    if($("#in_category").val()&&$("#in_article_name").val()&&$("#in_article_height_length").val()&&$("#in_article_height").val()&&$("#in_article_url").val())
    {
        var form_json = {
            category:$("#in_category").val(),
            name:$("#in_article_name").val(),
            width:$("#in_article_width").val(),
            height_length:$("#in_article_height_length").val(),
            height:$("#in_article_height").val(),
            price:$("#in_article_price").val(),
            image:$("#in_article_image").val(),
            url:$("#in_article_url").val()
        };

        $.post('/add',form_json);
        swal({title:'Thank You',text:'가구를 추가해주신것에 대해 감사드립니다',type:'success'}, function () {
            window.location.href="/search?page=1";
        });

    }else{
        swal('Warning!','이름, 가로, 세로, 높이, 링크 값이 필요합니다.');
    }
}
