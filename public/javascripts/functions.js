...
function add_on_off_conv(value) {
    var ret_value = false;
    if (value == 'on') {
        ret_value = true;
    }
...
function addBTN() {
    if ($("#meat").val() && $("#spicy").val() && $("#soup").val() && $("#in_article_name").val()) {
        var form_json = {
            meat: $("#add_meat").val(),
            spicy: $("#add_spicy").val(),
            soup: $("#add_soup").val(),
        ...
        $.post('/add', form_json);
        swal({ title: 'Thank You', text: '메뉴 추가 완료!', type: 'success' }, function() {
            window.location.href = "/";
        });

    } else {
        swal('Warning!', '정보를 빠짐없이 입력해주세요.');
    }
}
